...
컷 구분 없이 이어지는 나레이션 텍스트 하나로. TTS가 빠른 속도로 읽는다는 걸 감안해서 숨 쉴 틈 없이 이어지는 문장으로 써줘.
4단 구조를 지키되 그걸 체크리스트처럼 딱딱 끊어 나열하지 말고, 접속사와 흐름을 살려서 하나의 이야기처럼 자연스럽게
이어지도록 써줘 — 구조는 맞는데 문장이 뚝뚝 끊기는 게 제일 나쁜 결과야.

**분량과 원리 설명의 우선순위 — 중요**: 먼저 아래 "원리 설명" 규칙을 충실히 지켜서 완결된 대본을 쓰고, 그다음에
분량을 공백 포함 ${targetChars}자 내외(±30자)로 다듬어줘. 순서가 반대가 되면 안 돼 — 글자수부터 맞추려고
원리 설명 문장을 아예 빼거나 한 줄로 뭉뚱그리지 마. 제대로 설명하면 목표보다 길어져도 괜찮아, 그럴 땐 다른
군더더기 문장(수식어, 반복되는 표현)부터 줄여서 분량을 맞추고 원리 설명은 마지막까지 보존해줘.

추가로 아래 규칙들을 반드시 지켜줘:
- **원리 설명(가장 중요)**: 발상 전환 지점에서 "그래서 [해결책]을 합니다"라고 결과만 말하고 끝내지 마. 왜 그
  해결책이 실제로 작동하는지 — 물리적/기계적 인과관계를 한두 문장 더 붙여서 설명해줘. (나쁜 예: "속도를 감지하는
  장치를 달아서 브레이크가 걸리게 만듭니다" — 결과만 있고 원리가 없음. 좋은 예: "회전하는 추가 원심력으로 벌어지는
  정도를 이용해서 속도를 감지하고, 일정 속도를 넘으면 그 추가 레버를 밀어 브레이크를 작동시킵니다" — 왜/어떻게가
  들어있음.) 시청자가 "아, 그래서 되는구나"라고 납득할 수 있어야 해, "그렇다고 하니까 되나 보다"에서 끝나면 안 돼.
