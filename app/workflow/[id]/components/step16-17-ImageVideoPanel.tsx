...
//
// 2026-09-10 화풍 붕괴(Style Drift) 방지 강화:
...
// 방식(플랫 셀식 vs 수채/유화풍 붓칠), 렌더링 기법 자체가 씬마다 달랐다. 아래 4대 핵심 원칙 1번을
// "절대 고정 스타일" 수식어로 촘촘하게 강화하고, Flow(채팅형 프롬프트 UI라 별도 네거티브 프롬프트
// 입력칸이 없음)에서도 통하도록 금지 요소를 본문에 명시적 문장으로 박아넣었다. 상세 근거는
// workflow_content(DB) 문서 2026-09-10 로그 참고 — 이 패널 프롬프트 문구를 다시 고칠 일이 생기면
// 반드시 workflow_content 14번 행 마스터 지침 코드블록도 같이 갱신할 것(반대 방향도 마찬가지).
//
// 2026-09-10 화풍 붕괴 원인 추가 특정(재수정) — 대본의 극적인 조명/분위기 묘사가 원인:
// 위 강화만으로는 부족하다는 게 재확인됐다 — 진짜 원인은 대본(나레이션)의 극적인 연출 지문
// ("핀조명이 떨어지며", "어둠 속에서", "피눈물" 등)을 이미지 프롬프트로 옮길 때 그 단어를
// glowing/shadow/spotlight/cinematic 등으로 그대로 직역해버리는 것이었다 — AI가 이 단어들을
// "2D 평면이 아니라 3D·실사처럼 명암을 넣으라"는 신호로 오해한다. 대본 자체는 훌륭하고 수정
// 대상이 아니다 — 오직 "대본→imagePrompt 번역 단계"만 고친다: 금지 단어 목록을 명시하고, 그
// 극적인 뉘앙스는 2D 만화적 기호(플랫 별모양 버스트, 단색 어두운 배경 블록, 단순 눈물방울
// 아이콘 등)로 치환하도록 구체적 예시까지 박아넣었다. 아래 4대 핵심 원칙 1번 참고.
//
...
// (문장형 자막은 13번 나레이션·자막 단계가 SRT로 만들고 16번 렌더링에서 별도로 입힌다). 상세
// 근거는 workflow_content(DB) 문서 2026-09-10 로그 참고 — 이 패널 프롬프트 문구를 다시 고칠 일이
...
// 후반 합성/누끼 목적의 배경 단순화는 아니다). 상세 근거와 폐기 이력은 workflow_content(DB)
// 문서에 남아있다 — 이 패널 프롬프트 문구를 다시 고칠 일이 생기면 반드시 workflow_content
// 14번 행도 같이 갱신할 것(반대 방향도 마찬가지).
//
...
1. 비주얼 톤앤매너 (2026-09-10 강화, 원인 특정 후 재수정 — 대본의 극적 조명/분위기 묘사가 원인): "A single consistent art style, 2D flat vector illustration, simple Korean webtoon comic style, solid flat colors, crisp black line art, no shading, no gradients, minimal details, limited color palette (black, white, red, gold), bright plain solid-color background"로 통일한다. 모든 캐릭터는 예외 없이 스틱맨 몸(얇은 선 팔다리 + 단순한 동그란 머리) 비율로만 그린다 — 정상적인 인체 비율(어깨 넓은 몸통, 상세한 손가락·근육 등)로 그리지 않는다. 다음 단어는 imagePrompt에 절대 쓰지 않는다: glowing, shadow, illuminated, spotlight, macro, cinematic, 3D, realistic, photographic, gradient, watercolor, oil painting, film-noir. 대본의 극적인 조명/분위기 묘사(예: "핀조명이 떨어지며", "어둠 속에서", "피눈물")를 영어 프롬프트로 옮길 때 이 단어들을 그대로 직역하지 말고 2D 만화적 기호로 바꿔 표현한다 — 예: "핀조명이 비춘다" → 캐릭터 뒤에 그라데이션 없는 단색 플랫 별모양/방사형 버스트(flat solid-color starburst) 도형 배치, "어둠 속에서" → 그라데이션 없는 단색 어두운 배경 블록 + 캐릭터의 과장된 자세·표정으로 대체, "피눈물" → 작고 단순한 빨간 눈물방울 아이콘(사실적 묘사 없이). 대본 자체(나레이션·자막)는 수정 대상이 아니다 — 극적인 느낌은 이 번역 단계의 만화적 기호 치환으로만 살린다.
...
3. 사물 비유와 캐릭터 리액션의 결합 + 의미 전달용 텍스트 라벨 (2026-09-10 수정 — 실제 벤치마크 "경제학 똑똑" 재확인 결과 텍스트 완전 금지가 틀린 원칙이었음이 확인됨): 사물 단독 샷은 안 된다 — 반드시 캐릭터의 물리적 상호작용과 리액션이 결합된 구도로 짠다. 예: '무너지는 동전 탑'을 그릴 거라면, "동전 탑이 무너진다"가 아니라 "스틱맨 캔들러가 무너지는 동전 탑을 보며 머리를 쥐어뜯고 오열한다"처럼 반드시 캐릭터의 물리적 상호작용과 리액션이 결합된 구도로 짠다. 단, 의미가 헷갈릴 수 있는 사물·수치·비교 항목에는 짧은 한글 단어 라벨을 적극 붙인다 — 그래프의 축·눈금·항목명, 항아리/상자 등에 붙는 카테고리명, 금액 등(실제 벤치마크가 막대그래프의 "이자율(%)"·"대출금리"·"예금금리"나 항아리의 "고정지출"·"생활지출"·"비정기지출" 같은 라벨을 그림 안에 직접 쓰는 것을 확인함). 다만 문장형 캡션이나 나레이션 자막 문장은 이미지 안에 넣지 않는다 — 그건 13번(나레이션·자막) 단계가 SRT로 만들고 16번(렌더링)에서 별도로 입힌다.
...
[
  {
    ...
    "imagePrompt": "영어, Flow AI 이미지 생성용 완성된 프롬프트(캐릭터 묘사 + 직관적 사물 상황극 + 필요시 단어 수준 텍스트 라벨 + 미니멀 상황 배경 포함)",
