...
⚠️ needsVideoClip이 true인 장면의 imagePrompt 특별 규칙 (2026-09-13 실사고로 추가 — 검수 없이
넘어갔다가 사용자가 직접 발견: "지금 우리가 보유한 이미지로 이걸 못만드는데~~ 시작 이미지가
족같아"): 이 이미지는 나중에 Flow 영상 생성의 "시작 프레임"으로 그대로 쓰인다. 획일적으로 "항상
사건 직전만 그려라"는 기계적 규칙이 아니다 — 매 장면마다 videoPrompt에 적을 동작(떨어짐·충돌·
움직임 등)이 무엇인지 먼저 보고, 그 동작이 이 정지 이미지에서 자연스럽게 이어질 수 있는 상태가
어떤 것인지 장면별로 직접 판단해서 그린다. 대부분의 경우 이는 사건이 벌어진 "결과/클라이맥스"
순간이 아니라 사건이 "지금 막 일어나려는 찰나(그 직전)"를 뜻한다 — 결과 장면을 그리면 그
정지 이미지에서 더 이상 애니메이션할 동작이 안 남기 때문이다. 예: "네온사인이 이미 떨어져
영수증을 짓누르고 액체가 흘러나온 상태"가 아니라 "네온사인이 아직 공중에서 떨어지기 시작하는
순간"을 그린다. 다만 이건 일반적인 경우의 판단 근거일 뿐 — videoPrompt를 먼저 정하고, 그
동작이 실제로 나올 수 있는 시작 상태가 무엇인지를 장면 내용에 맞게 매번 스스로 판단한다.
