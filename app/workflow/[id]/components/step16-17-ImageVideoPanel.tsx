...
// 안된거 아니야???") 드러났다. 아래 4대 핵심 원칙 1번을 새 화풍으로 교체하고, 그 안에서 참조하던
// 스텝 번호(13번 나레이션·자막→12번, 16번 렌더링→15번)도 2026-09-11 재편에 맞춰 같이 갱신했다.
// **이 패널 프롬프트 문구를 다시 고칠 일이 생기면, 반드시 이 코드 파일과 workflow_content(DB) 13번
// 행 마스터 지침 코드블록을 "실제로 둘 다" 갱신할 것 — 한쪽만 고치고 "동기화 완료"라고 적어두는
// 실수를 반복하지 않는다. 가능하면 갱신 직후 이 패널에서 실제로 복사되는 텍스트를 직접 눈으로
// 확인해서 새 문구가 들어갔는지 검증할 것.**
//
// 2026-09-11 시간 커버리지 누락 방지 재확인 문구 추가:
// 위 87개 씬을 새 화풍으로 다시 만들기 전에, 그 87개 씬 자체가 SRT 전체(13:19)를 다 커버하지 못하고
// 12:00에서 멈춘 채 "완료"로 등록돼 있었다는 것도 함께 드러났다 — 마지막 1분 19초(클로징 멘트 포함)가
// 통째로 이미지 없이 비어있었다. 프롬프트의 "[최종: ...]" 완료 표시 지시는 이미 있었지만, 실제로
// 그 시점에 SRT 끝까지 도달했는지 재확인하라는 명시적 지시가 없어서 제미나이(또는 사람)가 스토리가
// 자연스럽게 끝나는 지점에서 조기에 완료로 착각했을 가능성이 크다. [분량 처리 방식] 섹션에 완료
// 표시 직전 재확인 문구를 추가했다 — 아래 참고.
//
// 2026-09-10 화풍 붕괴(Style Drift) 방지 강화(당시 배경, 위 2026-09-11 교체로 "플랫 벡터" 결론
// 자체는 폐기됐지만 원인 진단은 참고용으로 남겨둠):
// 실제 등록된 108개 장면(코카콜라 유닛)을 콘택트시트로 전부 확인한 결과, 씬마다 화풍이 마치
// 다른 사람이 그린 것처럼 심하게 갈라지는 문제가 발견됐다(사용자 지적: "그림체 모르니? 다른사람이
// 그린거 같은거") — 단순히 밝기/톤 문제가 아니라, 캐릭터 몸 비율(스틱맨 vs 정상 인체), 선화·음영
// 방식(플랫 셀식 vs 수채/유화풍 붓칠), 렌더링 기법 자체가 씬마다 달랐다.
//
// 2026-09-10 화풍 붕괴 원인 추가 특정(재수정) — 대본의 극적인 조명/분위기 묘사가 원인:
// 진짜 원인은 대본(나레이션)의 극적인 연출 지문("핀조명이 떨어지며", "어둠 속에서", "피눈물" 등)을
// 이미지 프롬프트로 옮길 때 그 단어를 glowing/shadow/spotlight/cinematic 등으로 그대로
// 직역해버리는 것이었다 — AI가 이 단어들을 "2D 평면이 아니라 3D·실사처럼 명암을 넣으라"는 신호로
// 오해한다. 2026-09-11: 화풍 자체를 디테일한 웹툰/코믹 일러스트로 바꾸면서, 이 단어들을 더 이상
// 금지하지 않고 오히려 적극 허용 — 대본의 극적 지문을 그대로 직역해도 되게 됐다(단, 2D 일러스트
// 렌더링 자체는 유지, 3D·사진처럼 보이면 안 됨).
//
// 2026-09-10 "텍스트 절대 금지" 원칙 정정 (실제 벤치마크 재확인 결과):
// "경제학 똑똑" 실제 영상(사회초년생 재테크편)을 시간대별로 다시 캡처해보니, 이자율 막대그래프에
// "이자율(%)"·"대출금리"·"예금금리" 같은 축/항목 라벨이 그려져 있고, 인물이 항아리에 돈을 넣는
// 상황극 장면에도 "고정지출"·"생활지출"·"비정기지출" 라벨이 항아리에 붙어있었다 — 데이터 장면뿐
// 아니라 상황극 장면에도 라벨이 쓰인다. 반면 화면 하단 나레이션 문장은 삽화가 아니라 별도로
// 얹은 자막바였다. 그래서 "텍스트 완전 배제"를 "의미 전달용 단어 라벨(그래프 항목명, 사물
// 카테고리명, 금액 등)은 적극 사용하되, 문장형 나레이션 자막은 이미지에 넣지 않는다"로 정정했다
// (문장형 자막은 12번 나레이션·자막 단계가 SRT로 만들고 15번 렌더링에서 별도로 입힌다).
//
// 2026-09-09 이미지 스타일 마스터 지침 전면 교체 (다크 은유/손 클로즈업/텍스트금지+회색배경 →
// 미니멀 스틱맨 인포그래픽 + 원샷 완성 배경):
...
// 유지하되 화풍만 미니멀 스틱맨(흰 원형 얼굴+점 두 개 눈)으로 단순화 (2) 텍스트는 위
// 2026-09-10 정정대로 의미 전달용 라벨은 허용하되 문장형 자막은 넣지 않는다 (3) 실존 인물도
// 손 클로즈업으로 피하지 않고 특징 없는 제네릭 스틱맨+소품으로 직접 등장 (4) 배경은 회색
// 단색이 아니라 장면 상황에 맞는 배경까지 한 프레임에 완성하는 "원샷(one-shot)" 방식(단,
// 후반 합성/누끼 목적의 배경 단순화는 아니다).
//
// 2026-09-09 "인물 필수 등장" 원칙 추가 + 영어 작성 순서 공식 명시: 위 규칙만으로 실제
// 10씬을 뽑아보니 6씬(60%)이 캐릭터 없이 배경/사물만 나오는 문제가 발견됐다(대본의
// [화면/음향 연출] 지문이 사물 중심 상징 컷을 자주 지시하는데, 이미지 프롬프트가 그걸
// 그대로 옮기면서 캐릭터를 누락시킴). 실제 벤치마크("S&P500 이렇게 사세요" 0/90/300/600초
// 실측)는 정반대로 캐릭터가 화면 대부분을 차지하고, 순수 그래프/사물 컷은 숫자 강조용으로
// 가끔만 낀다 — "인물 필수 등장 및 주어 전진 배치"(캐릭터를 프롬프트 맨 앞에, 사물은 그
// 다음)와 "사물 단독 샷 금지, 반드시 캐릭터 리액션과 결합" 원칙을 4대 핵심 원칙에 명시
// 추가했다. 이어서 AI가 여전히 배경 위주로 그리는 걸 원천 차단하기 위해, 영어 프롬프트
// 작성 순서를 [비주얼 톤앤매너] -> [캐릭터 외형 묘사] -> [캐릭터의 구체적인 표정과
// 행동(동사)] -> [주변 사물 및 배경]으로 명확한 공식으로 못박았다. 마지막으로 파서
// (`extractSceneArrays`)는 마크다운 코드펜스가 섞여도 원래 안전하지만(괄호 매칭으로
// `[...]`만 추출), 만에 하나를 대비해 프롬프트에도 "```json/``` 쓰지 말라"는 문장을
// 명시적으로 재차 못박았다.
//
// 2026-09-09 제미나이 응답 붙여넣기 → 자동 파싱 등록 기능 추가:
// 8) 프롬프트가 "구간(장면 25~35개)씩 만들고 '계속'으로 이어받는" 자기분할 방식이 되면서,
...
//    다 됐는지" 검증 가능하게 하는데, 여긴 그 완료 검증 표시가 빠져 있었다. 마지막 구간
...
1. 비주얼 톤앤매너 (2026-09-11 확정 — 디테일한 웹툰/코믹 일러스트 화풍, 이전 "완전 플랫 벡터" 방향 폐기): "High-quality 2D digital illustration, bold webtoon/comic art style, clean thick black outlines, dynamic cel shading and dramatic lighting, rich detailed background matching the scene's environment, full expressive color palette, glowing/spark/lighting effects allowed for dramatic emphasis"로 통일한다. 모든 캐릭터는 예외 없이 스틱맨 몸(얇은 선 팔다리 + 단순한 동그란 머리) 비율로만 그린다 — 정상적인 인체 비율(어깨 넓은 몸통, 상세한 손가락·근육 등)로 그리지 않는다. 다음 단어만 imagePrompt에 절대 쓰지 않는다(2D 손그림/일러스트 매체 자체를 벗어나는 표현이라 계속 금지): 3D, realistic, photographic, photorealistic, live-action, watercolor, oil painting, film-noir, macro. 반대로 glowing, shadow, illuminated, spotlight, gradient, cinematic은 더 이상 금지어가 아니다 — 대본의 극적인 조명/분위기 묘사(예: "핀조명이 떨어지며", "어둠 속에서", "피눈물")는 이제 2D 기호로 치환하지 않고 그대로 직역해도 된다(단, 항상 2D 일러스트 렌더링을 유지하고 3D·사진처럼 보이면 안 된다는 원칙은 그대로 지킨다).
...
[
  {
    ...
    "imagePrompt": "영어, Flow AI 이미지 생성용 완성된 프롬프트(캐릭터 묘사 + 직관적 사물 상황극 + 필요시 단어 수준 텍스트 라벨 + 상황에 맞는 디테일한 배경 포함)",
