...
[분량] 전체 나레이션 기준 약 15분 분량으로 작성한다. [TTS] 줄 텍스트만 합쳤을 때 공백 포함 최소 6,000자 이상은 반드시 채워라(목표 범위 6,000~7,500자, 15분 기준 검증된 값). 짧게 쓰고 나중에 늘리는 건 어려우니 최소선보다 넉넉하게 쓰고, 다듬어서 줄이는 건 나중에 해도 된다. [화면/음향 연출] 지시문은 이 글자수에 포함하지 않는다. 대본을 다 쓴 맨 마지막 줄에 [글자수: N자] 형식으로 [TTS] 텍스트만 합산한 글자수를 직접 세어서 표시해라.
