...
비용 절감을 위해 전체 장면을 다 영상으로 만들지 않는다. 시청자를 끌어모을 만큼 후킹이 강한 장면(콜드오픈은 물론, 본편 중 핵심 반전이 터지는 순간처럼 시선을 확 붙잡는 장면이면 챕터 어디든 해당될 수 있다)만 실제로 움직이는 영상 클립으로 만들고, 그 외 나머지 장면은 전부 정지 이미지 한 장에 줌인/줌아웃·클로즈업·화면 이동(패닝) 같은 단순한 카메라 효과만 얹어서 쓴다. 그러니:
- 영상으로 만들 만큼 후킹이 강하다고 판단한 장면은, 그 장면의 [화면/음향 연출] 줄 바로 앞에 [영상화]라는 줄을 단독으로 추가해라(형식은 아래 [출력 형식] 참고). 전체 대본에서 3~6개 정도로만 아껴서 표시해라 — 너무 많이 표시하면 비용 절감 취지에 어긋난다.
- [영상화] 표시가 없는 장면의 [화면/음향 연출]은 반드시 "정지된 한 장의 그림"만으로 완성되는 구도만 묘사해라 — 인물이 걷거나, 물체가 떨어지거나, 표정이 실시간으로 바뀌는 등 실제 움직임이 있어야만 성립하는 장면은 쓰지 마라. 대신 줌·클로즈업·패닝만으로도 강렬해 보이는 정적 구도(예: 극적인 조명 아래 놓인 사물, 인물의 결정적 표정을 한 컷으로 포착한 순간)로 묘사해라.
