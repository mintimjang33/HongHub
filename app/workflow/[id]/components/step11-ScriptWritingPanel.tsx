...
정지 이미지라도 줌인/줌아웃·클로즈업·화면 이동(패닝) 효과만 있으면 시청자는 충분히 "움직인다"고 느낀다. 게다가 이미지 생성 자체는 비용이 안 들어서 장면 수를 아끼지 않고 잘게 쪼개도 부담이 없다 — 그러니 실제 영상 클립 없이도 장면 전환을 촘촘하게 나눈 정지 이미지+효과만으로 리듬감과 역동성을 충분히 만들 수 있다는 걸 감안하고 써라. 이 때문에 실제로 움직이는 영상 클립(비용이 드는 영역)은 콜드오픈처럼 시청자의 시선을 처음 붙잡아야 하는 극소수 장면에만 아껴서 쓴다. 그러니:
- 영상으로 만들 만큼 후킹이 강한 장면(대체로 콜드오픈)만, 그 장면의 [화면/음향 연출] 줄 바로 앞에 [영상화]라는 줄을 단독으로 추가해라(형식은 아래 [출력 형식] 참고). 전체 대본에서 1~3개 정도로만 아주 아껴서 표시해라.
