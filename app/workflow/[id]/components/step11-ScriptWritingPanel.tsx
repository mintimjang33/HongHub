...
정지 이미지라도 줌인/줌아웃·클로즈업·화면 이동(패닝) 효과만 있으면 시청자는 충분히 "움직인다"고 느낀다. 게다가 이미지 생성 자체는 비용이 안 들어서 장면 수를 아끼지 않고 잘게 쪼개도 부담이 없다 — 그러니 실제 영상 클립 없이도 장면 전환을 촘촘하게 나눈 정지 이미지+효과만으로 리듬감과 역동성을 충분히 만들 수 있다는 걸 감안하고 써라. 이 때문에 실제로 움직이는 영상 클립(비용이 드는 영역)은 콜드오픈처럼 시청자의 시선을 처음 붙잡아야 하는, 정말 후킹이 강한 장면에만 아껴서 쓴다. 그러니:
- 영상으로 만들 만큼 후킹이 강한 장면(대체로 콜드오픈)만, 그 장면의 [화면/음향 연출] 줄 바로 앞에 [영상화]라는 줄을 단독으로 추가해라(형식은 아래 [출력 형식] 참고). 개수를 미리 정해두지 말고, 정말 필요하다고 판단되는 곳에만 최소한으로 표시해라.
- [영상화] 표시가 없는 나머지 모든 장면은 문장 단위로 더 잘게 끊어 장면을 촘촘하게 나눠도 된다 — 어차피 이미지는 비용이 안 들고, 잦은 컷 전환 자체가 역동성을 만들어준다. 각 장면의 [화면/음향 연출]은 반드시 "정지된 한 장의 그림"만으로 완성되는 구도만 묘사해라 — 인물이 걷거나, 물체가 떨어지거나, 표정이 실시간으로 바뀌는 등 실제 움직임이 있어야만 성립하는 장면은 쓰지 마라. 대신 줌·클로즈업·패닝만으로도 강렬해 보이는 정적 구도(예: 극적인 조명 아래 놓인 사물, 인물의 결정적 표정을 한 컷으로 포착한 순간)로 묘사해라.
- [영상화]로 표시한 장면만 인물/사물이 실제로 움직이거나 반응하는 동작을 묘사해도 된다.
...
- 위 [영상 제작 방식]에서 영상화 대상으로 고른 장면은, [화면/음향 연출] 줄 바로 앞에 아래처럼 단독 줄을 넣어라:
[영상화]
[화면/음향 연출] (이 장면은 실제 움직임을 묘사해도 됨)
[TTS] ...
표시가 없는 장면은 전부 정지 이미지로 처리된다는 뜻이다.
...
[TTS 발음 주의 — AI 음성이 이상하게 읽는 표현은 피하거나 한글로 풀어써라]
[TTS] 줄은 실제로 기계 음성(TTS)이 그대로 읽는다. 아래 유형은 오독·어색한 발음이 자주 나니 [TTS] 줄에는 쓰지 말고 풀어써라(연출 지시인 [화면/음향 연출] 줄에는 원래 표기로 써도 된다):
- 큰 숫자·소수점·연도+숫자 조합(예: "12,450km", "3.7배")은 자릿수 그대로 두지 말고 소리 나는 대로 한글로 풀어써라(예: "1만 2450킬로미터", "3.7배" → "3점 7배").
- 영어 약어·이니셜(NASA, GDP, EU, IMF 등)은 실제 읽는 발음대로 한글로 표기해라(예: "지디피", "아이엠에프"). 로마자 그대로 두면 알파벳을 한 글자씩 읽어버리는 경우가 많다.
- %, $, °, km/h, ㎡ 같은 기호·단위 표기는 전부 한글로 풀어써라("%"→"퍼센트", "$"→"달러", "°C"→"도").
- 이모지, 이모티콘, "ㅋㅋㅋ"·"ㅎㅎ" 같은 자모음 반복 표기는 쓰지 않는다.
- 괄호 안 부연설명·영어 병기는 [TTS] 줄에 넣지 마라 — TTS가 괄호까지 그대로 읽는다.
...
[분량 및 진행 방식 — 한 번에 전체를 다 쓰지 말고 아래 방식을 반드시 지켜라]
한 번의 응답에 전체 대본을 몰아서 쓰면 이야기가 짧고 압축되는 경향이 있다. 그러니 [콜드오픈] → [인트로] → [챕터 1] → [챕터 2] → ... → [아웃트로] 순서로, 한 응답에는 딱 하나의 섹션만 써라.
- 각 섹션의 [TTS] 텍스트 합계가 최소 700자 이상이 되도록 그 섹션 안에서 시대적 디테일·감정선·비유를 충분히 살려 써라(전체 목표가 아니라 지금 쓰는 이 섹션 하나만의 기준이다).
- 섹션 하나를 다 쓰면 그 응답 맨 마지막 줄에 정확히 이렇게만 적고 멈춰라: (다음 섹션 준비됨 — "계속"이라고 답하면 이어서 씁니다)
- 사용자가 "계속"이라고 답하면, 앞서 쓴 섹션들의 톤·사실·흐름과 자연스럽게 이어지도록 다음 섹션을 같은 방식으로 써라. 앞 섹션을 요약하거나 다시 쓰지 말고 그다음 내용만 새로 써라.
- 마지막 [아웃트로]까지 다 쓴 뒤에만 맨 마지막 줄에 [글자수: 전체 TTS 합계 N자]를 표시해라. 전체 목표는 6,000~7,500자(최소 6,000자 이상) — 부족하면 아직 끝내지 말고 방금 쓴 섹션을 더 늘려써라.
[화면/음향 연출] 지시문은 이 글자수에 포함하지 않는다.
