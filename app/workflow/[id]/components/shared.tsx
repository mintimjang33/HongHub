      ...
      >
        ...
        <div className="relative flex items-center justify-center bg-black min-h-[45vh]">
          ...
          {/* 2026-09-16(19차) 추가 — 브라우저 네이티브 재생시간 표시("0:06 / 0:06")는 밀리초까지
              ...
              2026-09-17(7차) 수정 — 사용자 지적: "상단길이가 맞으니까 6초로 하려고 하는거자나"
              + "뭐가 맞는건지 하나로 맞춰" — (5~6차)에서 이 자리에 브라우저 <video>가 읽은 파일
              전체 길이(오디오 트랙까지 포함한 컨테이너 기준, 예: 6.016초)를 보여줬는데, 같은
              화면 아래 "사용 구간"의 상한선(.mlt에 기록된, 실제 영상 프레임 기준 마지막 프레임
              위치 — 예: 5.960초)과 서로 다른 숫자라 어느 쪽이 맞는지 혼란을 줬다. ffprobe로
              직접 확인한 결과 영상 스트림의 실제 프레임 수(예: 144프레임×24fps=6.000초)가 곧
              사용 가능한 진짜 최대치이고, 컨테이너 전체 길이가 그보다 긴 건 오디오 트랙 끝의
              여유 패딩일 뿐 실제 영상 프레임이 아니다 — 즉 아래 "사용 구간" 쪽 숫자가 맞다.
              두 곳에서 서로 다른 값을 보여주지 않도록, .mlt 쪽 값(clipControl.info.sourceMaxMs)을
              구할 수 있으면(14번) 그걸 그대로 쓰고, 없으면(13번, .mlt 정보가 없는 경우) 어쩔 수
              없이 브라우저 값으로 대체한다 — 항상 하나의 숫자만 "원본 길이"로 보여준다. */}
          {videoTime && (
            <span className="absolute top-2 left-2 text-[13px] font-mono font-bold text-white bg-black/60 rounded px-2 py-1 pointer-events-none">
              재생 {msToClock(videoTime.current)} / 원본 길이 {msToClock(clipControl?.info?.sourceMaxMs ?? videoTime.duration)}
            </span>
          )}
          {hasNext && (
            <button
