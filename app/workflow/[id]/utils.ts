...
// 15번(썸네일 제작) — 2026-09-17 신설, 사용자 요청: "15단계는 롱폼일경우 썸네일을 만드는
// 과정". ⚠️ 이 단계를 추가하면서 desc에 "이번 콘텐츠에 맞는" 문구를 썼다가 아래 stepLink()
// 폴백 정규식(/생성|콘텐츠/)에 그대로 걸려서 "🎯 소스 발굴 → 콘텐츠 생성 탭" 링크가 또
// 겹쳐 떴다(사용자: "씨발 소스발굴또 생겼네... 도대체 뭐때문에 계속 겹쳐서 나오는거야"). 이
// 정확히 같은 사고가 계정/콘텐츠 설정(15→16번) 신설 때도 났었는데(위 주석) 또 반복됐다 —
// **새 단계를 만들 때마다 이 exact-match 판별 함수를 만들고 아래 stepLink() 제외 목록에
// 추가하는 걸 절대 빠뜨리지 말 것. desc 문구를 조심해서 "생성"·"콘텐츠" 단어를 피하는 것만으론
// 부족하다 — 나중에 desc가 또 수정될 수 있으니 exact-match 함수로 원천 차단해야 한다.**
export function isThumbnailStep(step: Step): boolean {
